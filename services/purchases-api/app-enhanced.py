...
def lambda_handler(event, context):
    """
    Handler mejorado para AWS Lambda que maneja requests HTTP para compras
    ...
    try:
        ...
        # UPDATE CART ITEM
            ...
            try:
                ...
                if not book or book.get("stock", 0) < quantity:
                    return {
                        "statusCode": 400,
                        "headers": headers,
                        "body": json.dumps(
                            {
                                "error": "Insufficient stock",
                                "available_stock": book.get("stock", 0) if book else 0,
                            }
                        ),
                    }
        ...
        # CHECKOUT PROCESS
            ...
            try:
                    ...
                    if cart_item["sk"].startswith("ITEM#"):
                        ...
                        if book.get("stock", 0) < quantity:
                            return {
                                ...
                                "body": json.dumps(
                                    {
                                        "error": f'Insufficient stock for {cart_item.get("title", "Unknown")}',
                                        "available_stock": book.get("stock", 0),
                                    }
                                ),
                            }
                ...
                for item in order_items:
                    ...
                    new_stock = book.get("stock", 0) - item["quantity"]
